...
public class BoardPanel extends JPanel {
    ...
    private void paintTile(boolean isTileWhite, int tileNumber) {
        if (isTileWhite) graphics.setColor(Color.decode(WHITE_TILE_COLOR));
        else             graphics.setColor(Color.decode(BLACK_TILE_COLOR));

        graphics.fillRect(xPanelCoordinate, yPanelCoordinate, TILE_WIDTH, TILE_HEIGHT);

        if (!isTileWhite) {
            graphics.setColor(Color.decode(TILE_NUMBER_STRING_COLOR));
            graphics.drawString(String.valueOf(tileNumber), xPanelCoordinate, yPanelCoordinate + 10);
        }
    ...
    private void paintPiece(boolean isPieceWhite, boolean isPieceQueen) {
        if (isPieceWhite) graphics.setColor(Color.decode(WHITE_PIECE_COLOR));
        else              graphics.setColor(Color.decode(BLACK_PIECE_COLOR));

        graphics.fillOval(xPanelCoordinate + TILE_WIDTH/2 - PIECE_SIZE/2,
                            yPanelCoordinate + TILE_HEIGHT/2 - PIECE_SIZE/2,
                               PIECE_SIZE, PIECE_SIZE);


        if (isPieceQueen) {
            graphics.setColor(Color.decode(QUEEN_MARK_COLOR));
            graphics.fillOval(xPanelCoordinate + TILE_WIDTH/2 - QUEEN_MARK_SIZE/2,
